...
static int backend_valid(const osaura_processor_bus_backend *backend) {
    return backend && backend->task_count && backend->is_program &&
           backend->foreground_pid && backend->wake && backend->sleep;
}
...
static int build_route(void) {
    ...
    if (foreground < count && foreground < 16u &&
        g_backend.is_program(foreground, g_backend_context)) {
    ...
    for (uint32_t pid = 0; pid < count; ++pid) {
        ...
        if (!g_backend.is_program(pid, g_backend_context)) continue;
        if (route_contains(pid)) continue;
...
static int wake_current(void) {
    ...
    if (g_backend.wake(pid, g_backend_context) != 0) return -2;
...
int osaura_processor_bus_publish(const osaura_processor_bus_change *change) {
    ...
    if (build_route() != 0) {
        osaura_processor_bus_reset();
        return -4;
    }
    if (g_bus.route_count == 0u) return 0;
    return wake_current();
}
...
const osaura_processor_bus_info *osaura_processor_bus_get_info(void) {
    return g_initialized ? &g_bus : (const osaura_processor_bus_info *)0;
}
...
uint32_t osaura_processor_bus_route_pid(uint32_t order_index) {
    ...
    uint32_t shift = order_index * OSAURA_PROCESSOR_BUS_ROUTE_BITS_PER_PID;
    return (uint32_t)((g_bus.route_bits >> shift) & 0xfull);
}
