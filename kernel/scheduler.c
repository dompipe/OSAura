...
void osaura_scheduler_init(void) {
    ...
    int jx_candidate_ok = jx_book_ok && osaura_jx_runtime_queue_book(
        (const void *)(uintptr_t)osaura_jx_next_book, osaura_jx_next_book_size) == 0;
...
void osaura_scheduler_start(void) {
    ...
    if (!osaura_security_policy_self_test()) {
        g_initialized = 0u;
        g_running = 0u;
        return;
    }
...
const char *osaura_scheduler_task_name(uint32_t task_id) {
    if (task_id >= OSAURA_TASK_MAX || g_tasks[task_id].state == OSAURA_TASK_UNUSED) return "UNUSED";
...
int osaura_scheduler_set_task_state_as(uint32_t subject, uint32_t task_id,
                                       osaura_task_state state) {
...
int osaura_scheduler_background_as(uint32_t subject, uint8_t terminal_id, uint32_t *task_id) {
    ...
    r.subject = subject;
    r.terminal_id = terminal_id;
    int rc = dispatch_job(OSAURA_JOB_HOT_BACKGROUND, &r);
...
int osaura_scheduler_foreground_as(uint32_t subject, uint8_t terminal_id, uint32_t *task_id) {
    ...
    r.subject = subject;
    r.terminal_id = terminal_id;
    int rc = dispatch_job(OSAURA_JOB_HOT_FOREGROUND, &r);
    if (rc == 0 && task_id) *task_id = r.task_id;
    return rc;
}
int osaura_scheduler_wake_as(uint32_t subject, uint32_t task_id) {
    ...
    r.subject = subject;
    r.task_id = task_id;
    return dispatch_job(OSAURA_JOB_HOT_WAKE, &r);
}
...
uint32_t osaura_scheduler_foreground_task(uint8_t terminal_id) {
    osaura_job_hot_request r = {0};
    r.terminal_id = terminal_id;
    return dispatch_job(OSAURA_JOB_HOT_GET_FG, &r) == 0 ? r.task_id : OSAURA_TASK_NONE;
}
uint32_t osaura_scheduler_background_count(void) {
    osaura_job_hot_request r = {0};
    return dispatch_job(OSAURA_JOB_HOT_BG_COUNT, &r) == 0 ? r.value : 0u;
}
uint32_t osaura_scheduler_background_task(uint32_t stack_index) {
    osaura_job_hot_request r = {0};
    r.stack_index = stack_index;
    return dispatch_job(OSAURA_JOB_HOT_BG_AT, &r) == 0 ? r.task_id : OSAURA_TASK_NONE;
}
