...
static int storage_allowed(uint32_t subject, uint64_t right) {
    return subject == 0u || osaura_security_check(subject, right);
}
...
static int byte_request(const osaura_block_driver *driver,
                        const osaura_storage_request *r,
                        uint64_t *lba,
                        uint32_t *blocks) {
    if (!driver || !r || !driver->block_size || !r->bytes) return -1;
    if ((r->offset % driver->block_size) != 0u ||
        (r->bytes % driver->block_size) != 0u) return -2;
...
static int shadow_read1(void *c, void *o) {
    (void)c;
    osaura_storage_request *r = (osaura_storage_request *)o;
    if (!r || !storage_allowed(r->subject, OSAURA_CAP_STORAGE_READ)) return -9;
    osaura_block_driver *d = request_driver(r);
    if (!d || !r->buffer) return -1;
    if (!range_ok(d, r->lba, 1u)) return -2;
    return d->read(d->context, r->lba, 1u, r->buffer);
}

static int shadow_write1(void *c, void *o) {
    (void)c;
    osaura_storage_request *r = (osaura_storage_request *)o;
    if (!r || !storage_allowed(r->subject, OSAURA_CAP_STORAGE_WRITE)) return -9;
    osaura_block_driver *d = request_driver(r);
    if (!d || !r->const_buffer) return -1;
    if (!(d->capabilities & OSAURA_BLOCK_CAP_WRITE) || !d->write) return -2;
    if (!range_ok(d, r->lba, 1u)) return -3;
    return d->write(d->context, r->lba, 1u, r->const_buffer);
}

static int shadow_readn(void *c, void *o) {
    (void)c;
    osaura_storage_request *r = (osaura_storage_request *)o;
    if (!r || !storage_allowed(r->subject, OSAURA_CAP_STORAGE_READ)) return -9;
    osaura_block_driver *d = request_driver(r);
    if (!d || !r->buffer) return -1;
    if (!range_ok(d, r->lba, r->blocks)) return -2;
    return d->read(d->context, r->lba, r->blocks, r->buffer);
}

static int shadow_writen(void *c, void *o) {
    (void)c;
    osaura_storage_request *r = (osaura_storage_request *)o;
    if (!r || !storage_allowed(r->subject, OSAURA_CAP_STORAGE_WRITE)) return -9;
    osaura_block_driver *d = request_driver(r);
    if (!d || !r->const_buffer) return -1;
    if (!(d->capabilities & OSAURA_BLOCK_CAP_WRITE) || !d->write) return -2;
    if (!range_ok(d, r->lba, r->blocks)) return -3;
    return d->write(d->context, r->lba, r->blocks, r->const_buffer);
}

static int shadow_append_unbound(void *c, void *o) {
    (void)c;
    (void)o;
    return -8;
}
...
static int shadow_readat(void *c, void *o) {
    ...
    uint32_t blocks;
    if (!d || !r->buffer) return -1;
    int rc = byte_request(d, r, &lba, &blocks);
    if (rc != 0) return rc;
    return d->read(d->context, lba, blocks, r->buffer);
}
...
static int shadow_writeat(void *c, void *o) {
    ...
    int rc = byte_request(d, r, &lba, &blocks);
    if (rc != 0) return rc;
    return d->write(d->context, lba, blocks, r->const_buffer);
}

static int shadow_commit(void *c, void *o) {
    (void)c;
    osaura_storage_request *r = (osaura_storage_request *)o;
    if (!r || !storage_allowed(r->subject, OSAURA_CAP_STORAGE_WRITE)) return -9;
    osaura_block_driver *d = request_driver(r);
    if (!d) return -1;
    if (!(d->capabilities & OSAURA_BLOCK_CAP_FLUSH)) return 0;
    if (!d->flush) return -2;
    return d->flush(d->context);
}
...
int osaura_block_register(const osaura_block_driver *driver, uint32_t *device_id) {
    if (!driver || !driver->name || !driver->read || !driver->block_size ||
        !driver->block_count || !(driver->capabilities & OSAURA_BLOCK_CAP_READ)) return -1;
...
int osaura_block_get_info(uint32_t device_id, osaura_block_info *info) {
    if (!info || device_id >= OSAURA_BLOCK_DEVICE_MAX || !g_devices[device_id].used) return -1;
...
static int dispatch_storage(uint8_t shadow, osaura_storage_request *r) {
    return osaura_hot_dispatch_opcode(
        osaura_hot_opcode(OSAURA_HOT_BANK_STORAGE, shadow), r);
}

int osaura_block_read_as(uint32_t subject, uint32_t device_id, uint64_t lba,
                         uint32_t blocks, void *buffer) {
    osaura_storage_request r = {0};
    r.subject = subject;
    r.device_id = device_id;
    r.lba = lba;
    r.blocks = blocks;
    r.buffer = buffer;
    return dispatch_storage(blocks == 1u ? OSAURA_STORAGE_READ1 : OSAURA_STORAGE_READN, &r);
}

int osaura_block_write_as(uint32_t subject, uint32_t device_id, uint64_t lba,
                          uint32_t blocks, const void *buffer) {
    osaura_storage_request r = {0};
    r.subject = subject;
    r.device_id = device_id;
    r.lba = lba;
    ...
    return dispatch_storage(blocks == 1u ? OSAURA_STORAGE_WRITE1 : OSAURA_STORAGE_WRITEN, &r);
}

int osaura_block_read_at_as(uint32_t subject, uint32_t device_id, uint64_t offset,
    ...
    r.bytes = bytes;
    r.buffer = buffer;
    return dispatch_storage(OSAURA_STORAGE_READAT, &r);
}

int osaura_block_write_at_as(uint32_t subject, uint32_t device_id, uint64_t offset,
                             uint32_t bytes, const void *buffer) {
    osaura_storage_request r = {0};
    r.subject = subject;
    r.device_id = device_id;
    r.offset = offset;
    r.bytes = bytes;
    r.const_buffer = buffer;
    return dispatch_storage(OSAURA_STORAGE_WRITEAT, &r);
}
...
int osaura_block_flush_as(uint32_t subject, uint32_t device_id) {
    ...
    r.subject = subject;
    r.device_id = device_id;
    return dispatch_storage(OSAURA_STORAGE_COMMIT, &r);
}

int osaura_block_read(uint32_t device_id, uint64_t lba, uint32_t blocks, void *buffer) {
    return osaura_block_read_as(0u, device_id, lba, blocks, buffer);
}
int osaura_block_write(uint32_t device_id, uint64_t lba, uint32_t blocks, const void *buffer) {
    return osaura_block_write_as(0u, device_id, lba, blocks, buffer);
}
int osaura_block_read_at(uint32_t device_id, uint64_t offset, uint32_t bytes, void *buffer) {
    return osaura_block_read_at_as(0u, device_id, offset, bytes, buffer);
}
int osaura_block_write_at(uint32_t device_id, uint64_t offset, uint32_t bytes,
                          const void *buffer) {
    return osaura_block_write_at_as(0u, device_id, offset, bytes, buffer);
}
int osaura_block_flush(uint32_t device_id) {
    return osaura_block_flush_as(0u, device_id);
}
